...
def _resolve_project_root() -> Path:
    """Resolve repository root targeted by the squad run.

    Resolution order:
    1. `AURAXIS_PROJECT_ROOT` explicit path override.
    2. `<platform>/repos/<AURAXIS_TARGET_REPO>` (default: auraxis-api).
    3. Legacy fallback when ai_squad is inside the repo root.
    """
    explicit_project_root = os.getenv("AURAXIS_PROJECT_ROOT", "").strip()
    if explicit_project_root:
        resolved = Path(explicit_project_root).expanduser().resolve()
        if resolved.exists():
            return resolved
        raise RuntimeError(
            f"AURAXIS_PROJECT_ROOT points to a non-existent path: {resolved}"
        )
